...
hand = handTracker()
sign_methods_list = ['isThumbsUp', 'isPointingUp', 'isBird', 'isOkay', 'isFingerGun', 'isPeace', 'isVictory', 'isO']
...
def test_pointingUp():
    ...
    expected = [False] * len(sign_methods_list)
    expected[1] = True
    for i in range(len(sign_methods_list)):
        method = getattr(test_hand, sign_methods_list[i])
        if callable(method):
            result = method()
            assert result == expected[i], f'Failed on {sign_methods_list[i]}'
